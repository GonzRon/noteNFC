...
| **The dispatch trampoline** — `NfcDispatchActivity` | The *shape* (translucent, UI-less, `singleTop`, `excludeFromRecents`, hostile-extras guard, exactly two accepted actions, hand off to one renderer) is a documented pattern; the code reaches `AppGraph`, `MainActivity` and `Resolution` (arch §6.2) |
