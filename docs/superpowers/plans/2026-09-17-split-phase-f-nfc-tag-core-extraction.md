...
| `nfc-core/…/NdefSize.kt` | **NoteTag** `core/src/main/kotlin/com/loosecannon/notetag/core/nfc/NdefSize.kt` (whole file, 14 lines) | — (Phase E) | NoteTag `b0ec89c` | package line; KDoc says "writer" not "planner"; `require(records.isNotEmpty())` added (F-2) |
...
| `nfc-core/src/test/…/NdefSizeTest.kt` | **NoteTag** `core/src/test/…/notetag/core/nfc/NdefSizeTest.kt` (37 lines) | — | NoteTag `b0ec89c` | neutral identity; the 49 B case keeps its body length; the zero-size case replaced by an `assertFailsWith` refusal case (F-2) |
...
`NdefSizeTest.kt` — NoteTag's four size cases (`b0ec89c`) plus the F-2 refusal case, on a neutral identity of the same length as the one that produced the 49 B figure:
...
3. **`## Public API`** — the signatures of Tasks 2–4 exactly as the "Produces" blocks list them, grouped by module, each with its one-line KDoc summary; one line notes that `WriteResult.TooSmall` (what `write` reports) and `CapacityVerdict.TooSmall` (what `fit` predicts) are distinct types, always written qualified.
...
## Review round (nfc-tag-core `fb45aa0` → `4ca1a1e`, 2026-09-17)

The whole-branch review of the finished library returned "With fixes"; one fix round and one scoped re-review closed it. Task 4's `TagWriter`/`TagIo`/`WriteRoute` blocks above are the shipped shapes **as first written**; the library at FINAL differs from them in three ratified-API-shape amendments the controller made under the review's Critical/Important findings, recorded in target §4.2 (product-split `5dc41fb`) and **pending the owner's ratification** at the Phase F hand-off — zero consumers exist, each is one revert: (1) `TagInspection.existingRecords` + `unreadable: String?` → one `read: TagRead` (`Readable(records)` | `Unreadable(reason, cause)`), so an unreadable tag can no longer flatten to an empty one and be overwritten without a question (C1/I8); (2) `WriteResult.Failed(reason, cause = null, attempted = true)`, false only at the two pre-radio refusals, so a consumer's retain/remove rule no longer string-matches prose (I1); (3) `lock(tag, expected)` on `TagWriter`, `TagIo` and `RealTagIo` — it reads the tag fresh and locks only on structural equality, so invariant 9 is code, not a KDoc sentence (I2). Also: `FLAG_READER_SKIP_NDEF_CHECK` on the scan's word list; the allow file matched as exact path + fixed-substring fragment; the scan's paths widened to the three build scripts and the catalog (I4/P4, I5, I7); `NdefRecordData.toString()`; `FakeTagIo`'s impossible default replaced and its `lock`/`write` arguments captured; README invariants renumbered to §4.3 with honest proof lines, six NEW provenance rows (24), the `projectDir` snippet, a v0.2.0 deferred list (I6 prose false positives, I9 presence-check delay, M7, M9 CI without `check`/lint, M10, P6 NFC permission, P7, the empty-fragment allow guard). Counts at FINAL: nfc-core 50, nfc-android unit 10, connected 10 on `emulator-5554`, 0 failures; 11 commits; 43 files. Target §4.3 (P1: the "no API shape" claim narrowed; P2: invariant 6 is the consumer's) and §4.4 (P3) amended in the same docs commit. Evidence: the Phase F section at product-split, closing line unchanged.
