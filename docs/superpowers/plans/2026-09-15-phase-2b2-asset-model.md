...
sealed interface AssetProblem { data object NameRequired; data object BadCurrency; data object NegativePrice; data class BadDate(val field: String);
...
**Files:** Modify `entities/AssetEntity.kt` (new columns; `ForeignKey(AssetEntity::class, ["id"], ["parent_asset_id"], onDelete = RESTRICT)`, `Index("parent_asset_id")`), `dao/AssetDao.kt` (`deleteAll` unchanged is unsafe with a self-RESTRICT → delete children-first: `DELETE … WHERE parent_asset_id IS NOT NULL` repeatedly until zero rows, then the rest, in a `@Transaction` — or order by depth; document), `Migrations.kt` (+`MIGRATION_3_4`: recreate `asset` from `4.json`, copy v3 columns with `status = CASE status WHEN 'RETIRED' THEN 'ARCHIVED' ELSE status END`, drop, rename, indexes), `AppDatabase.kt` (v4), `Mappers.kt`, `RoomRepositories.kt`, `di/AppGraph.kt` + `FakeGraph.kt` (`retireAsset`, `deleteAsset`; `SCHEMA_VERSION = 4`; `.addMigrations(…, MIGRATION_3_4)`), `4.json`; Tests `Migration3To4Test`, `Migration1To4Test`, `AssetDaoTest` (parent RESTRICT, deleteAll with a tree), `RestoreProofTest` (tree survives). Fix `:app` call sites of `CreateAsset`/`UpdateAsset` (ViewModels) minimally so it compiles — the editor UI is Task 5.
