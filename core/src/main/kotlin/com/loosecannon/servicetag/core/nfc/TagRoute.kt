...
/**
 * `servicetag://tag/<uuid>` — resolves exactly as if the tag had been scanned (deep-link contract).
 * Navigation only; the id is validated by shape here and by existence in `ResolveTag`.
 */
object TagRoute {
    const val SCHEME: String = "servicetag"
    ...
    fun parse(scheme: String?, host: String?, pathSegments: List<String>): TagPayload? {
        ...
        val id = pathSegments.singleOrNull()
            ?: return TagPayload.Malformed("servicetag://tag needs exactly one path segment, got ${pathSegments.size}")
