...
class DeepLinkRouteTest {
    ...
    @Test fun linkRoute() { assertEquals(DeepLink.Link(LinkId(id)), DeepLinkRoute.parse("servicetag", "link", listOf(id))) }
    ...
    @Test fun badIdsAreMalformedNeverExceptions() {
        ...
        assertIs<DeepLink.Malformed>(DeepLinkRoute.parse("servicetag", "link", listOf(id.uppercase())))
