    ...
    Scaffold(
        ...
        LazyColumn(modifier = Modifier.padding(padding)) {
            if (state.needsBackup) {
                item { BackupNudge(onExport = onBackup, modifier = Modifier.padding(16.dp)) }
            }
            if (state.assets.isEmpty()) {
                item { FirstRun(onNewAsset = onNewAsset, onScan = onScan) }
            } else {
                item {
                    SectionHeader(title = "Current", modifier = Modifier.padding(horizontal = 16.dp))
                }
                items(state.assets, key = { it.id.value }) { asset ->
                    CurrentRow(asset = asset, onClick = { onOpenAsset(asset.id.value) })
                    HorizontalDivider(
                        thickness = 1.dp,
 ...
 */
@Composable
private fun CurrentRow(asset: Asset, onClick: () -> Unit) {
    Row(
